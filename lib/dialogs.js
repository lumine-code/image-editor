...
function createDialogBackdrop(document) {
...
function createDraggableDialog(document, title) {
...
function createSliderControl(document, config) {
...
function createButtonContainer(document, cancelText = "Cancel", applyText = "Apply") {
...
function createSelectionInfo(document, text) {
...
function createNumberInput(document, config) {
  const container = document.createElement("div");
  container.className = "dialog-slider-container";
...
function createCheckbox(document, id, labelText, checked = false) {
  const container = document.createElement("div");
  container.className = "dialog-slider-container";
...
function createQuickButtons(document, values, onClick, formatter = (v) => `${v}`) {
  const container = document.createElement("div");
  container.className = "dialog-slider-container";
...
function createPreviewSourceCanvas(document, imageElement, maxSize = 1280) {
...
function createPreviewPanel(document, options) {
  const {
    sourceImage,
    maxSourceSize = 1280,
    debounce = 60,
  ...
  const infoElement = createSelectionInfo(document, "");
  let sourceCanvas = createPreviewSourceCanvas(document, sourceImage, maxSourceSize);
  ...
  const refreshSource = (values = lastValues) => {
    sourceCanvas = createPreviewSourceCanvas(document, sourceImage, maxSourceSize);
    renderNow(values);
  };

  const resizeObserver = new document.defaultView.ResizeObserver(() => renderNow(lastValues));
...
function setupEscapeHandler(document, backdrop, cleanup) {
...
function showAdjustmentDialog(
  ...
  emitter,
  previewOptions = null,
  trackCleanup = (cleanup) => cleanup,
) {
  const backdrop = createDialogBackdrop(document);
  const { dialogElement, cleanup: dialogCleanup } = createDraggableDialog(document, title);
  ...
  const selectionInfo = createSelectionInfo(document, "");
  ...
  sliders.forEach((config) => {
    const { container, slider } = createSliderControl(document, config);
  ...
  if (previewOptions) {
    previewPanel = createPreviewPanel(document, {
      ...previewOptions,
  ...
  // Create buttons
  const { buttonContainer, cancelButton, applyButton } = createButtonContainer(document);
  let escapeHandler = null;

  const fullCleanup = trackCleanup(() => {
  ...
  escapeHandler = setupEscapeHandler(document, backdrop, fullCleanup);
...
module.exports = {
  forDocument(document, trackCleanup = (cleanup) => cleanup) {
    if (!document?.defaultView) {
      throw new TypeError("Image editor dialogs require a live Document");
    }
    return {
      createDialogBackdrop: () => createDialogBackdrop(document),
      createDraggableDialog: (title) => createDraggableDialog(document, title),
      createSliderControl: (config) => createSliderControl(document, config),
      createButtonContainer: (...args) => createButtonContainer(document, ...args),
      createSelectionInfo: (text) => createSelectionInfo(document, text),
      createNumberInput: (config) => createNumberInput(document, config),
      createCheckbox: (...args) => createCheckbox(document, ...args),
      createQuickButtons: (...args) => createQuickButtons(document, ...args),
      createPreviewPanel: (options) => createPreviewPanel(document, options),
      setupEscapeHandler: (...args) => setupEscapeHandler(document, ...args),
