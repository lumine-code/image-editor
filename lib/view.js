...
module.exports = class ImageEditorView {
  ...
  async updateImageURI(options = {}) {
    ...
    try {
      ...
      if (!currentLoad.cancelled) this.noteShownFile(filePath, stats);
    } catch (error) {
      if (!currentLoad.cancelled) {
  ...
  blurImage(blurLevel) {
    ...
    this.runFilterOperation({
      ...
      apply: (ctx, area) => canvasFilters.blurRegion(ctx, area, blurLevel * 2),
      defer: true,
      progressLabel: "blur",
    });
  }
  ...
  autoAdjustColors() {
    this.runFilterOperation({
      describe: (target) => `Auto color adjustment applied to ${target}`,
  ...
  runFilterOperation({ describe, apply, defer = false, progressLabel = null }) {
    ...
    if (this.filterInProgress) {
      lumine.notifications.addWarning("Still working on the last change");
      return;
    }
  ...
  commitCanvas(
    ...
    canvas.toBlob((displayBlob) => {
      if (this._destroyed || !displayBlob) {
        ...
        this.historyManager.releaseEntry(entry);
        this.hideSpinner();
        if (onError) onError();
        return;
      }
  ...
  noteShownFile(filePath, stats) {
    this.shownFile = stats ? { path: filePath, mtimeMs: stats.mtimeMs, size: stats.size } : null;
  }
  ...
  async loadFromHistory(historyEntry) {
    ...
    const url = await this.historyManager.urlFor(historyEntry);
    ...
    // The history owns this URL and releases it with the entry, so the image
    // element must not take it over.
    this.setImageSource(url, { own: false });
