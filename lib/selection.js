...
// Get the complete selection rectangle, including any part beyond the image.
function getSelectionBounds(selectionStart, selectionEnd) {
  const left = Math.round(Math.min(selectionStart.x, selectionEnd.x));
  const top = Math.round(Math.min(selectionStart.y, selectionEnd.y));
  const width = Math.round(Math.abs(selectionEnd.x - selectionStart.x));
  const height = Math.round(Math.abs(selectionEnd.y - selectionStart.y));
...
// Get the part of the selection that intersects the image.
function getSelectionArea(
  ...
  if (hasVisibleSelection) {
    const bounds = getSelectionBounds(selectionStart, selectionEnd);
...
function copyToClipboard(imageElement, left, top, width, height, onError) {
  if (width === 0 || height === 0) {
