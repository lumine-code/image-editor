...
module.exports = class ImageEditor {
  ...
  handleFileEvents() {
    ...
    const reconcile = (force = false) => {
      if (this.file === file) this.reconcileFile(force);
    };
    this.fileSubscriptions.add(
      file,
      file.onDidChange(() => reconcile(true)),
      file.onDidInvalidate(() => reconcile()),
      file.onDidError((error) => console.error("Unable to watch image", error)),
    );
    file.ready.then(
      () => reconcile(),
      () => {},
  ...
  reconcileFile(force = false) {
    if (this.destroyed || this.fileOperationDepth || !this.file) return;
  ...
  setPath(filePath) {
    ...
    this.file = watchFile(filePath);
    this._title = null;
    if (this.editorView?.shownFile) this.editorView.shownFile.path = filePath;
    if (this.editorView?.lastSelfWrite) this.editorView.lastSelfWrite.path = filePath;
    this.handleFileEvents();
  ...
  shouldPromptToSave() {
    return (
      this.fileState !== FileState.UNMODIFIED && lumine.config.get("core.promptOnCloseDirtyBuffer")
    );
  }
  ...
  noteExternalChange() {
    const locallyModified = this.editorView?.isModified() || false;
    if (this.fileState === FileState.REMOVED && !locallyModified) {
  ...
  confirmExternalChange() {
    if (this.editorView?.isModified() || this.fileState !== FileState.UNMODIFIED) {
      this.setFileState(FileState.CONFLICTED);
    }
  }

  didChangeHistoryModified(modified) {
    if (this.fileState === FileState.CONFLICTED || this.fileState === FileState.REMOVED) return;
  ...
  didObserveUnchangedFile() {
    this.externalChangePending = false;
    if (this.fileState === FileState.CONFLICTED) {
      this.setFileState(this.editorView?.isModified() ? FileState.MODIFIED : FileState.UNMODIFIED);
    }
  }
