...
// Make an arbitrary image-space rectangle the canvas. Drawing the complete
// source at the inverse offset preserves pixels inside the rectangle and
// leaves out-of-bounds parts transparent.
function resizeCanvasToArea(sourceCanvas, left, top, width, height, canvasPool = null) {
  const canvas = canvasPool
    ? canvasPool.getCanvas(width, height)
    : document.createElement("canvas");
  canvas.width = width;
