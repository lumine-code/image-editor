...
class HistoryManager {
  ...
  /**
   * The encoded pixels of an entry, once they exist.
   *
   * The blob, deliberately, and not a URL over it. A URL has to be revoked by
   * exactly one owner, and the history is the wrong one to be: the view can
   * still be displaying a frame when a save resets the history, and the
   * free-rotate dialog holds one across a whole preview session. Handing back
   * the blob lets the view mint a URL it owns outright, under the same rule as
   * every other source it shows.
   *
   * @returns {Promise<Blob|null>} null if the frame never encoded
   */
  async blobFor(entry) {
  ...
  /**
   * Take back a slot whose pixels never arrived.
   *
   * Distinct from releasing one: an entry that failed to encode was never a
   * state anybody can return to, so leaving it in place would mark the image
   * modified over a change that is not there and put a dead frame under the
   * cursor. Only the newest slot can be withdrawn — an older one has states
   * after it whose positions must not shift.
   */
  abandonEntry(entry) {
    if (!entry) return;
  ...
  // Drop an entry's pixels and unblock anyone waiting on them.
  releaseEntry(entry) {
    if (!entry || entry.released) return;
    entry.released = true;
    entry.blob = null;
