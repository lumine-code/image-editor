  ...
  try {
    ...
    canvas.toBlob(async (blob) => {
      try {
        ...
        if (onSuccess) onSuccess(filePath, getFileStats(filePath));
      } catch (error) {
...
async function saveImageAs(paneItem, imageElement, defaultPath, onSuccess, onError, onCancel) {
  try {
    const result = await lumine.workspace.showSaveDialogForPaneItem(paneItem, {
    ...
    canvas.toBlob(async (blob) => {
      try {
        ...
        if (onSuccess) onSuccess(savePath, getFileStats(savePath));
      } catch (error) {
        if (onError) onError(error);
      }
    }, mimeType);
